...
class StaticInfoGenerator:
    ...
    def __init__(self, output_dir: str = None):
        if output_dir is None:
            # Use absolute path resolution from script location
        ...
        print("📊 Enhanced with Hockey Reference integration")
    ...
    def generate_static_context(self, game_id: str) -> str:
        ...
        print(f"🏠 Home: {game_info['home_team']} ({game_info['home_team_name']})")
        print(f"✈️  Away: {game_info['away_team']} ({game_info['away_team_name']})")
        
        # Get standings
        standings = self._get_standings()
        
        # Get enhanced rosters
        print("\n👥 Getting enhanced rosters...")
        rosters = self._get_enhanced_rosters(game_id, game_info)
        
        # Build enhanced static context
        static_context = {
            'game_id': game_id,
        ...
        print(f"📊 {static_context['player_count']} players with enhanced data")
        return filepath
    ...
    def _get_enhanced_rosters(self, game_id: str, game_info: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"✅ Enhanced {rosters['enhanced_count']} players with Hockey Reference data")
    ...
    def _enhance_player_data(self, player: Dict) -> Dict[str, Any]:
        ...
        # Try to enhance with Hockey Reference
    ...
    def _get_hockey_reference_data(self, player_name: str, player_id: str) -> Optional[Dict]:
        """Get Hockey Reference data for a player"""
        # Check cache first
        if player_id in self.player_cache:
            return self.player_cache[player_id]
        
        try:
            # Search Hockey Reference for player
    ...
    def _search_hockey_reference(self, player_name: str) -> Optional[Dict]:
        """Search Hockey Reference for player stats"""
        try:
            # For MVP demo, return mock enhanced data
            # In production, this would scrape Hockey Reference
            mock_data = {
            ...
            print(f"📊 Mock Hockey Reference data for {player_name}")
            return mock_data
    ...
    def _generate_player_context(self, hr_data: Dict) -> str:
        ...
        # In production, would analyze HR data to create context
